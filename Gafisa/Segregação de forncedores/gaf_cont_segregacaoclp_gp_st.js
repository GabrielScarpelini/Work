...
function onRequest(ctx) {
    ...
    var form = ui.createForm({
        ...
        var valorCurto = sublist.addField({
            id: 'custpage_valor_curto',
            type: ui.FieldType.CURRENCY,
            label: 'Valor a Curto Prazo'
        }).updateDisplayType({displayType: ui.FieldDisplayType.DISABLED})
        
        var valorLongo = sublist.addField({
        ...
        // try{
            ...
            var busca = search.create({
                ...
                if(result.getValue({name: "status", join: "installment"}) == 'Não pago' ) {
                    ...
                    if (dataApuracao < dataSeg){
                        objResultados.curtoPrazo += Number(result.getValue({name: "amount", join: "installment"}));
                    }else{
                        objResultados.longoPrazo += Number(result.getValue({name: "amount", join: "installment"}));
                    }
            ...
            for (var i = 0; i < resultados.length; i++){
                ...
                sublist.setSublistValue({
                    id: "custpage_valor",
                    line: i,
                    value: resultados[i].curtoPrazo + resultados[i].longoPrazo
                })
                
                sublist.setSublistValue({
                    id: "custpage_valor_curto",
                    line: i,
                    value: resultados[i].curtoPrazo
                })
                
                sublist.setSublistValue({
                    id: "custpage_valor_longo",
                    line: i,
                    value: resultados[i].longoPrazo
                })

                // sublist.setSublistValue({
                //     id: "custpage_prestacoes",
                //     line: i,
                //     value: contCurto + contLongo
                // })
