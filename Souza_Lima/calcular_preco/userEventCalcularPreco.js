...
define(["require", "exports", "N/log", "N/record", "N/runtime", "N/search", "N/config", "./rsc_functions"], function (require, exports, log_1, record_1, runtime_1, search_1, config_1, rsc_functions_1) {
    "use strict";
    ...
    var beforeLoad = function (context) {
        ...
        form.clientScriptModulePath = "./ClientCalcularPreco.js";
        if (context.type == context.UserEventType.EDIT) {
            // Adiciona o botao de calcular preco
            form.addButton({
                label: "Calcular Preço",
                functionName: "calcularPreco",
                id: "custpagecalcprecobutton"
            });
        }
    };
    ...
    var afterSubmit = function (ctx) {
        ...
        var inicio_periodo_noturno = companyInformationRec.getText("custrecord_lrc_inicio_periodo_noturno");
        //Log.error("inicio_periodo_noturno", inicio_periodo_noturno);
        var fim_periodo_noturno = companyInformationRec.getText("custrecord_lrc_fim_periodo_noturno");
        //Log.error("fim_periodo_noturno", fim_periodo_noturno);
        var inicio_periodo_noturno_adnot = companyInformationRec.getText("custrecord_rsc_inicio_adnot");
        //Log.error("inicio_periodo_noturno_adnot", inicio_periodo_noturno_adnot);
        var fim_periodo_noturno_adnot = companyInformationRec.getText("custrecord_rsc_fim_adnot");
        //Log.error("fim_periodo_noturno_adnot", fim_periodo_noturno_adnot);
        var user = runtime_1.default.getCurrentUser();
        var total_taxas = parseFloat(String(companyInformationRec.getValue("custrecord_lrc_total_taxas")));
        ...
        var _loop_1 = function () {
            try {
                ...
                if (!escala_lookup["custrecord_lrc_horas_por_dia"] && escala_lookup["custrecord_lrc_horas_por_dia"].length > 0) {
                    errors.push({
                        "name": "MISSING FIELD",
                        "description": 'O campo custrecord_lrc_horas_por_dia é necessário no registro LRC @ Escala',
                        "line": line
                    });
                    return "continue";
                }
                ...
                log_1.default.error("Horas_por_dia_lookup", horas_por_dia_lookup);
                //
                var checkIsFullday = false;
                ...
                if (horas_por_dia_trabalhadas == 24) {
                    horas_por_dia_trabalhadas = 23;
                    checkIsFullday = true;
                }
                log_1.default.error("horario_de_almoco", horario_de_almoco);
                log_1.default.error("horas_por_dia_trabalhadas", horas_por_dia_trabalhadas);
                ...
                if (checkIsFullday) {
                    sublistFieldsText_1["custcol_lrc_hora_saida_so"] = String((horario_de_saida.horas + 1) + ":" + horario_de_saida.minutos);
                }
                else {
                ...
                //QUANTIDADE (ADICIONADO DIA 24/06/2022)            
                var quantity = Number(record.getSublistValue({
                    sublistId: "item",
                    fieldId: "quantity",
                    line: line
                }));
                ...
                log_1.default.error("variaveis_salario", variaveis_salario_1);
                ...
                log_1.default.error("dias_trabalhados_mes_ben", dias_trabalhados_mes_ben);
                ...
                log_1.default.error("variaveis_add_noturno", variaveis_add_noturno_1);
                ...
                log_1.default.error("alimentacao cliente", alimentacao_cliente);
                ...
                log_1.default.error("total_impostos", total_impostos);
                ...
                log_1.default.error("total_rate", total_rate);
                // set lineFields
                //Log.error("sublistFieldsValue", sublistFieldsValue);
                //Log.error("sublistFieldsText", sublistFieldsText);


                log_1.default.error("sublistFieldsValue", sublistFieldsValue_1);
                log_1.default.error("sublistFieldsText_1", sublistFieldsText_1);
                Object.keys(sublistFieldsValue_1).forEach(function (key) {
                    record.setSublistValue({
                        sublistId: "item",
                        fieldId: String(key),
                        line: line,
                        value: sublistFieldsValue_1[key]? Number(sublistFieldsValue_1[key]): 0
                    });
                });
                ...
                //PARTE FEITA NÃO OFICIAL (24/06/2022)
                var amount = sublistFieldsValue_1["rate"]*quantity;
                log_1.default.error("amount", amount);
                record.setSublistValue({
                    sublistId: "item",
                    fieldId: "amount",
                    line: line,
        ...
        if (errors.length == 0) {
            ...
            //Log.error("p", p);
            var subsidiaria_taxa_operacional = parseFloat(String(companyInformationRec.getValue("custrecord_lrc_taxa_operacional")));
            //Log.error("sub_taxa_operacional", subsidiaria_taxa_operacional);
            var subsidiaria_reserva_tecnica = parseFloat(String(companyInformationRec.getValue("custrecord_lrc_reserva_tecnica")));
            //Log.error("sub_reserva_tecnica", subsidiaria_reserva_tecnica);
            var subsidiaria_lucro = parseFloat(String(companyInformationRec.getValue("custrecord_lrc_lucro")));
