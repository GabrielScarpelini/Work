 ...
 */
 ...
 (query) => {
         ...
         function getCalculo(idOperacao, period){
                 _loadAllValues();
                 var sql = 'select substr(subsidiary.name,1,4) subsidiary, custentity_rsc_term_cl_suspensiva inicioApropriacao, ' +
                     ' ' + getComponentes(idOperacao, period) + ' from subsidiary, job\n' +
                     'where 1 = subsidiary.custrecordtpemp and job.custentity_rsc_aprop_subsidiaria = subsidiary.id and job.id in (21910, 21952, 21890, 21922) order by 1';
                 log.error({title:'SQL Calculo', details: sql});
